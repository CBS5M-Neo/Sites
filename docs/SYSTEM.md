...
### Выбор модели и контроль доступности

Neo умеет работать с четырьмя моделями провайдера B.AI через DeepSeek Harness:
`glm-5.3-flash` (умолчание), `hy3`, `qwen3.8-flash`, `mimo-v2.5`.

Источник выбора (по убыванию приоритета):

1. аргумент `--model` / выбор в форме issue / поле `model` в workflow_dispatch;
2. упоминание модели в тексте заявки (`parseModelFromText`);
3. env `NEO_MODEL`;
4. модель исходного сайта (для правок);
5. `glm-5.3-flash`.

Перед использованием `pickWorkingModel()` отправляет модели короткую пробу
(«Ответь: OK», таймаут 90 с). Если модель не отвечает — **лимит исчерпан или API
лежит** — pipeline автоматически перебирает остальные модели списка и берёт
первую живую; сбой фиксируется в логе. Если генерация всё же упала посреди
работы (например, лимит кончился в процессе), `generate-site.js` и `apply-edit.js`
повторяют шаг на следующей рабочей модели. Фактическая модель сохраняется в
`sites.json` (поле `model`) и видна в каталоге и в отчёте issue.

Технически модель переключается вторым `--patch` к `dsh --profile headless`,
переопределяющим `agent-default-model` (см. `modelPatchPath` в `lib/common.js`).
...
| `model` | модель ИИ, реально генерировавшая сайт |
